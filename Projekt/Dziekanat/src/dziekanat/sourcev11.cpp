...
int main(int argc, char *argv[]) {
	...
	if (argc<3||(string)argv[1] != "-i") {
		...
		for (int i = 0; i < argc - 2; i++) {
			...
			list_info lista;
			if (dodaj_do_listy(file_name, lista)) {
				wypiszDoPliku(lista);
				licznik_true++;
			}
			else licznik_false++;
	...
	cout <<"Poprawnie przetworzono "<< licznik_true <<" plikow."<< endl;
	cout << "Nie udalo sie przetworzyc " << licznik_false << " plikow." << endl;
	system("pause");
	return 0;
}
...
bool dodaj_do_listy(string file_name, list_info &lista) {
	...
	if (!plik_wejscia.is_open()) {
		cout << "Blad otwarcia pliku:	"<< file_name << endl;
		return false;
	}
	//========================================
